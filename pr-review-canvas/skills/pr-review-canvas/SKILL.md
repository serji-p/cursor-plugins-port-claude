...
4. Open the result for the user:
   - Simplest: `open /tmp/pr-review-{number}.html` (macOS) so it opens in the default browser.
   - Or serve it and open in a browser / browser MCP tool:
     ```bash
     cd /tmp && python3 -m http.server 8432 --bind 127.0.0.1
     ```
     Run this backgrounded, then open `http://127.0.0.1:8432/pr-review-{number}.html`. If a browser MCP (Claude-in-Chrome, preview server, Playwright MCP) is connected, navigate it there; otherwise hand the URL/file path to the user.

   **Why a fixed port and `cd /tmp` when serving:** Background shells have no TTY, so Python buffers its startup message ("Serving HTTP on...") indefinitely — using port 0 means you can never read which port was chosen. And `--directory /tmp` works but `cd /tmp` is more robust across Python versions. If port 8432 is taken, try 8433, 8434, etc.
