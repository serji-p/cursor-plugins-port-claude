...
description: Review the current branch for bugs, intent fit, and test coverage; run or write tests; commit focused work; open or update a PR. Use for "review and ship this", finalizing a branch before review, or taking a change from done-coding to PR.
---
...
1. Gather context: diff against base branch, uncommitted changes, recent commits, changed files, and the user's stated intent (from the current session or commit messages).
2. Run targeted tests for changed behavior. If no focused tests exist, decide whether to add them or document the gap.
3. Review for correctness, regressions, security, and intent fit. Dispatch parallel subagents (via the Agent tool) for larger diffs.
