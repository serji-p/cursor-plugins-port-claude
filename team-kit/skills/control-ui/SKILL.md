...
> Claude Code note: if a browser MCP tool is connected (e.g. Claude-in-Chrome, a Playwright MCP, or a preview server), prefer it for clicks/snapshots/screenshots before hand-rolling the scripts below.
...
- Creating before/after evidence for the `/verify` skill.
