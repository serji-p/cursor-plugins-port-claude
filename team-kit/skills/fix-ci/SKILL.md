---
name: fix-ci
description: Find failing PR checks, inspect logs or external check links, and apply focused fixes. Use when "CI is red", a PR check failed, or you need a fast iterative path to green checks.
---
...
2. Inspect failed jobs and extract the first actionable error. Use GitHub Actions logs when available (`gh run view <run-id> --log-failed`); otherwise use the check link to identify the failing command or service.
